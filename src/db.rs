...
impl FileEventType {
    pub fn to_string(&self) -> String {
        match self {
            FileEventType::Create => "create".to_string(),
            FileEventType::Modify => "modify".to_string(),
            FileEventType::Delete => "delete".to_string(),
        }
    }
...
impl Database {
    ...
    pub fn update_file(
        ...
        if rows_updated == 0 {
            ...
            warn!("Path {} not found for update in 'files' table, attempting to create.", path_str);
            self.create_file(path_str, file_type, hash)
        } else {
            // File was updated, now fetch it to return
            debug!("Path {} updated successfully in 'files' table, fetching.", path_str);
            self.conn.query_row(
    ...
    pub fn mark_event_processed(&self, processed_event_id: &str, processed_event_path: &str, processed_event_created_at: i64) -> rusqlite::Result<usize> {
        debug!(
